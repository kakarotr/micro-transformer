...
def count_text_rows(data_dir: str, workers: int = 8) -> int:
    ...
    with ProcessPoolExecutor(max_workers=workers) as ex:
        return sum(ex.map(count_file, map(str, files)))


def count_pretrain_token(data_dir: str, tokenizer: PreTrainedTokenizerFast, workers: int = 8):
