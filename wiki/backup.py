...
from utils.client import deepseek_client
from utils.prompt import wiki_table_prompt
from utils.schemas import TableSchema
...
class WikiPageParser:
    ...
    def parse(self, title: str, lang: str = "zh"):
        ...
        raw_content = self._get_raw_content(title=title, lang=lang)
        # 清洗摘要
        summary = self._clean_summary(summary=raw_content.sections[0])
        sections.append(WikiSection(title="summary", blocks=[SectionBlock(type="text", content=summary)]))

        for section in raw_content.sections[1:]:
            pass
    ...
    def _clean_summary(self, summary: wtp.WikiText):
        ...
        return self._clean_by_pattern(text=summary_content)
...
def clean_cell_text(cell_content: wtp.WikiText | str | None):
    """
...
def table_handler(table: wtp._table.Table):
    ...
    headers = [clean_cell_text(item) for item in raw_data[0]]
    rows = raw_data[1:]
    input_data = {"headers": headers, "rows": []}
    for row in rows:
        if row:
            items = []
            for item in row:
        ...
        messages=[
            {
                ...
                "content": wiki_table_prompt.format(
                    json_schema=json.dumps(TableSchema.model_json_schema(), ensure_ascii=False),
    ...
    result = response.choices[0].message.content or '{"lines": []}'
    # result = '{"lines": ["aaaaaaaa", "aaaaaaaa", "aaaaaaaa"]}'
    table_content = TableSchema.model_validate_json(result)
    return "\n".join(table_content.lines)
...
def replace_table(text: wtp.WikiText):
    for table in text.tables[::-1]:
        raw_data = table.data(span=True)
        headers = [clean_cell_text(item) for item in raw_data[0]]
        rows = raw_data[1:]
        input_data = {"headers": headers, "rows": []}
