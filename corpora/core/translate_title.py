...
def translate_title(n_threads: int = 10):
    chunks = get_chunks(sql="select id, title from wiki_pages where title_zh is null", n_threads=n_threads)
...
def process(chunk):
    ...
    with Live(refresh_per_second=10) as live:
        ...
        for row in chunk:
            ...
            cursor.execute("update wiki_pages set title_zh = %s where id = %s", (result.text, row[0]))
            conn.commit()
